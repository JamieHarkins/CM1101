...
def execute_go(direction):
    ...
    if too_heavy == False:
        ...
        print("You are carrying to much and won't be able to move rooms, drop something to move")
...
def check_weight(inv_items):
    global too_heavy
    total_weight = 0
    for weight in inv_items:
        total_weight = total_weight + float(weight["mass"])
    if total_weight > 3.0:
        too_heavy = True

def check_win(room_items):
    if len(room_items) == 6:
...
def main():
    ...
    while True:
        ...
        too_heavy = False
        check_win(current_room["items"])
        print_room(current_room)
        print_inventory_items(inventory)

        # Show the menu with possible actions and ask the player
        check_weight(inventory)
